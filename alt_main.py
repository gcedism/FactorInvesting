...
vol_period = 60  # Volatility Period
mom_period = 12  # Momentum Period
reg_period = 60  # Regression window
...
def run_model(X, Y):
  ...
  index = []
  cols = [
    x for x in data.columns if x not in ['market', 'vol', 'momentum', 'SPY']
  ]
  for col in cols:
    y = Y.loc[:, col]
  ...
  sel_stocks = list(new_results.sort_values('avg', ascending=False).index)[:40]
